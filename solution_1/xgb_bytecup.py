...
trains = pd.read_csv("pre/train.csv")  # 注意自己数据路径
...
tests = pd.read_csv("pre/test.csv")  # 注意自己数据路径

print tests.head()

# test_id = range(len(tests))
test_f = tests.iloc[:, :15]
# test_qid = tests.userID
test_userID = tests.userID
# test_uid = tests.creativeID
test_creativeID = tests.creativeID
